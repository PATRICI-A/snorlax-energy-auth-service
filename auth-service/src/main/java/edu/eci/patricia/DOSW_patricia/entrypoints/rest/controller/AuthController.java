...
public class AuthController {
    ...
    @Operation(
            ...
            description = "Triggered by the registration service immediately after a new user account has been " +
                    "created. Receives the user's institutional email, generates a cryptographically random " +
                    "6-digit OTP, stores it in Redis with a 10-minute TTL, and publishes an email event via " +
                    "RabbitMQ so the notification service delivers the code to the user's inbox. This endpoint " +
                    "does NOT return the OTP — it only confirms the code was dispatched. The user must then " +
                    "call POST /verify-otp to activate their account."
    )
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "503",
                    description = "The User Service is temporarily unreachable. Retry after a short delay.",
                    content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
                            schema = @Schema(implementation = ErrorResponse.class)))
    })
    ...
    @Operation(
            ...
            description = "Completes the password recovery flow started by POST /forgot-password. Validates the " +
                    ...
                    "not expired, the new password is hashed with BCrypt and updated in the User Service. " +
                    "The code is single-use and deleted from Redis immediately upon success. " +
    ...
    @Operation(
            ...
            description = "Allows an authenticated user to change their own password. Requires a valid JWT Bearer " +
                    ...
                    "BCrypt and saved. This endpoint requires the user to know their current password — it is " +
                    "not a recovery flow. Use POST /forgot-password for forgotten passwords."
    )
    @ApiResponses({
            @ApiResponse(responseCode = "200",
